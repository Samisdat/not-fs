...
module.exports = function(vfsTree){
    ...
    var useRealFileSystem = function(){

        var trace = stackTrace.get(getCaller(useRealFileSystem));
        var caller = trace[0];
        console.log(caller.getFileName(), caller.isToplevel())
        var caller = trace[1];
        console.log(caller.getFileName(), caller.isToplevel())
        if('module.js' === caller.getFileName() && true === caller.isToplevel()){
            return true;
        }
    ...
    fs.existsSync = function(path){
        console.log('existsSync', arguments);  
        return;
    ...
    fs.openSync = function(path, flags, mode){
        console.log('openSync', useRealFileSystem());
        if(true === useRealFileSystem()){
    ...
    fs.readdirSync = function(path){
        console.log('readdirSync', arguments);  
        return;
    ...
    fs.realpathSync = function(path, cache){
        if(true === useRealFileSystem){
            return realpathSync(path, cache);
        }
