...
Tree.prototype.removeNode = function(inodeNumber){
    ...
    if (true === node.isDir() && true === this.hasChildren(inodeNumber)){
...
Tree.prototype.isDir = function(pathName){
...
Tree.prototype.rename = function(oldPath, newPath, addMissingDirs){
    ...
    if(parentInodeNumverOfOldPath !== parentInodeNumverOfNewPath){
        ...
        for (var i = 0, x = this.tree[parentInodeNumverOfOldPath].length; i < x; i += 1){
            var children = [];
            if (inodeNumber === this.tree[parentInodeNumverOfOldPath][i]){
                continue;
            }
            children.push(this.tree[parentInodeNumverOfOldPath][i]);
        }

        this.tree[parentInodeNumverOfOldPath] = children;
