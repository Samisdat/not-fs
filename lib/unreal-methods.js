...
unrealFs.setTree = function(tree){
    unrealFsTree = tree;
};
...
unrealFs.appendFileSync = function(file, data, options){
    ...
    if(false === unrealFsTree.exists(file)){
         unrealFsTree.addFile(file);   
    }

    var content = unrealFsTree.getByFqn(file).getContent();

    unrealFsTree.getByFqn(file).setContent(content + data);   
...
unrealFs.exists = function(path, callback){
    var exists = unrealFsTree.exists(path);
...
unrealFs.existsSync = function(path){
    return unrealFsTree.exists(path);
};
...
unrealFs.readdirSync = function(path){
    if(false === unrealFsTree.exists(path)){
        throw new Error('ENOENT: no such file or directory, scandir \''+ path + '\'');
    }

    var chilren = unrealFsTree.getByFqn(path).getChildren();
...
unrealFs.readFileSync = function(file, options){
    ...
    return unrealFsTree.getByFqn(file).getContent();
};
...
unrealFs.renameSync = function(oldPath, newPath){
    ...
    if(false === unrealFsTree.exists(oldPath)){
         throw new Error(oldPath + ' does not exists.'); 
    }

    var node = unrealFsTree.getByFqn(oldPath);

    unrealFsTree.remove(oldPath);

    if(true === node.isDir()){
       unrealFsTree.addDir(newPath); 
    }
...
unrealFs.statSync = function(path){
    var node = unrealFsTree.getByFqn(path);
...
unrealFs.writeFileSync = function(file, data, options){
    ...
    unrealFsTree.getByFqn(file).setContent(data);   
